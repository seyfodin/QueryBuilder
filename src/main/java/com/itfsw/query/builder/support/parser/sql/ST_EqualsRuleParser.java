...
public class ST_EqualsRuleParser extends AbstractSqlRuleParser {
    private final EnumDBType dbType;

    public ST_EqualsRuleParser(EnumDBType dbType) {
        this.dbType = dbType;
    }

    public boolean canParse(IRule rule) {
        return EnumOperator.ST_EQUALS.equals(rule.getOperator());
    }

    public Operation parse(IRule rule, JsonRuleParser parser) {
        if (dbType.equals(EnumDBType.SPATIALITE))
            return new Operation(new StringBuffer("ST_Equals(").append(rule.getField()).append(", GeomFromEWKT(?)) = 1"), rule.getValue());
        else
            return new Operation(new StringBuffer("ST_Equals(").append(rule.getField()).append(", GeomFromEWKT(?))"), rule.getValue());
    }
}