...
public class ST_DisjointRuleParser extends AbstractSqlRuleParser {
    private final EnumDBType dbType;

    public ST_DisjointRuleParser(EnumDBType dbType) {
        this.dbType = dbType;
    }

    public boolean canParse(IRule rule) {
        return EnumOperator.ST_DISJOINT.equals(rule.getOperator());
    }

    public Operation parse(IRule rule, JsonRuleParser parser) {
        if (dbType.equals(EnumDBType.SPATIALITE))
            return new Operation(new StringBuffer("ST_Disjoint(").append(rule.getField()).append(", GeomFromEWKT(?)) = 1"), rule.getValue());
        else
            return new Operation(new StringBuffer("ST_Disjoint(").append(rule.getField()).append(", GeomFromEWKT(?))"), rule.getValue());
    }
}
